...
from tilediiif.infojson import DEFAULT_ID_BASE_URL
...
def test_infojson_fails_with_invalid_arguments(argv, message):
    result = subprocess.run(['infojson'] + argv,
...
def test_infojson_generates_expected_output_on_stdout(dzi_path, info_json):
    result = subprocess.run([
        'infojson', 'from-dzi', '--stdout', '--id-base-url', ID_BASE_URL,
        '--id', 'MS-ADD-00269-000-01075', dzi_path],
...
@pytest.mark.parametrize('options, dzi_path, expected_content, expected_path, \
expected_id_attr', [
    ...
    [['--id', 'foo',
      '--path-template', 'some-dir/{identifier-shard}/{identifier}_info.json'],
     DATA_DIR / 'MS-ADD-00269-000-01075.dzi',
     DATA_DIR / 'MS-ADD-00269-000-01075.info.json',
...
def test_infojson_writes_expected_output_file(
        chdir, dzi_path, options, expected_content, expected_path,
        expected_id_attr, tmp_data_path, monkeypatch):
    if chdir:
...
def test_indent(indent):
    result = subprocess.run([
        'infojson', 'from-dzi', '--stdout', '--indent', str(indent),
        DATA_DIR / 'MS-ADD-00269-000-01075.dzi'],
        capture_output=True, encoding='utf-8', check=True)
...
def test_indent_0_disables_indentation():
    result = subprocess.run([
        'infojson', 'from-dzi', '--stdout', '--indent', '0',
        DATA_DIR / 'MS-ADD-00269-000-01075.dzi'],
        capture_output=True, encoding='utf-8', check=True)
