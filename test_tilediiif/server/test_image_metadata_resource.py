...
def test_image_resource_base_redirects_to_info_json(
        client: testing.TestClient, url):
...
@pytest.mark.parametrize('config, sendfile_header_name, info_json_path', [
    [Config(file_transmission=FileTransmissionType.INDIRECT),
     'X-Accel-Redirect', 'foo/info.json'],
    [Config(file_transmission=FileTransmissionType.INDIRECT,
            info_json_path_template='prefix/{identifier}/info.json'),
     'X-Accel-Redirect', 'prefix/foo/info.json'],
    [Config(file_transmission=FileTransmissionType.INDIRECT,
            sendfile_header_name='x-sendfile'),
     'x-sendfile', 'foo/info.json'],
    [Config(file_transmission=FileTransmissionType.INDIRECT,
            data_path='/var/images'),
     'X-Accel-Redirect', '/var/images/foo/info.json'],
    [Config(file_transmission=FileTransmissionType.INDIRECT,
            info_json_path_template='{identifier-shard}/{identifier}/'
                                    'info.json',
            data_path='/var/images'),
     'X-Accel-Redirect', '/var/images/b1/71/foo/info.json'],
])
...
@pytest.mark.parametrize('config', [
    Config(file_transmission=FileTransmissionType.DIRECT, data_path=TEST_DIR,
