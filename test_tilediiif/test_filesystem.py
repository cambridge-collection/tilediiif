...
@pytest.mark.parametrize(
    "base_path", [Path("/"), Path("relative/base"), Path("/abs/base")]
)
@pytest.mark.parametrize(
    "sub_path, expected_sub_dirs",
    [
        [Path(""), []],
        [Path("foo"), []],
        [Path("foo/bar"), ["foo"]],
...
def test_ensure_sub_directories_exist(
    base_path, sub_path, expected_ensure_dir_paths, mock_ensure_dir_exists
):
...
@pytest.mark.parametrize(
    ...
    [
        ["abc/../foo", None, None, 'path contains a ".." (parent) segment: abc/../foo'],
        [
            "abc/../foo",
            AssertionError,
...
def test_validate_relative_path_rejects_invalid_paths(
    ...
    kwargs = {
        k: v for k, v in dict(prefix=prefix, exc_cls=exc_cls).items() if v is not None
    }
