...
def test_config_creation_from_kwargs(config_toml_expected_config):
    ...
    assert (
        c.image_path_template
        == "foo-{identifier-shard}/{identifier}/"
        "{image-shard}/{region}-{size}-{rotation}-{quality}.{format}"
    )
    assert c.info_json_path_template == "foo-{identifier-shard}/{identifier}/info.json"
...
@pytest.mark.parametrize(
    "path, msg",
    [
        [DATA_DIR / "missing.toml", f"Unable to read {DATA_DIR / 'missing.toml'}: "],
        [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {
                "tilediiif": {
                    "server": {"indirect-file-transmission": {"unknown": "foo"}}
                }
            },
            None,
            (
