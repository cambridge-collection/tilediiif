...
def _is_relative_uri_path(uri):
    if not all(x is None for x in (uri.scheme, uri.authority,
...
def get_id_url(id_base_url, id):
    ...
    if not base.is_absolute():
        raise CLIError(
            f'invalid --id-base-url {id_base_url!r}: url is not absolute')
...
def _create_templated_file_output_method(data_path: Path, path_template: str):
    ...
    def _output_to_file(info_json_content: bytes, identifier: str):
        ...
        try:
            ...
            raise CLIError(
                f'failed to write output to {path} because {e}') from e
...
def _get_default_id(dzi_path):
    ...
    raise CLIError(f'no --id is specified so --id is derived from <dzi-file>, '
...
def run(args):
    ...
    identifier = (args['--id'] if '--id' in args
                  else _get_default_id(args['<dzi-file>']))
